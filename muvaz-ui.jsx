// muvaz.online — UI atoms (shadcn-style, mobile-first)
// Zinc neutrals + muvaz tomato accent. Inter throughout.

// ── tokens ───────────────────────────────────────────────────
const mBg        = '#fafafa';   // zinc-50
const mWhite     = '#ffffff';
const mBorder    = '#e4e4e7';   // zinc-200
const mBorder2   = '#d4d4d8';   // zinc-300 (pressed / strong)
const mText      = '#18181b';   // zinc-900
const mSubtext   = '#52525b';   // zinc-600
const mMuted     = '#71717a';   // zinc-500
const mMutedBg   = '#f4f4f5';   // zinc-100
const mMutedBg2  = '#e4e4e7';   // zinc-200 hover
const mAccent    = '#18181b';   // black (monochrome mode)
const mAccentHov = '#3f3f46';
const mAccentBg  = '#f4f4f5';
const mRing      = 'rgba(24,24,27,0.15)';
const mRadius    = '8px';
const mRadiusSm  = '6px';
const mRadiusLg  = '12px';
const mShadow    = '0 1px 3px rgba(0,0,0,0.07), 0 1px 2px rgba(0,0,0,0.04)';
const mShadowMd  = '0 4px 6px -1px rgba(0,0,0,0.07), 0 2px 4px -1px rgba(0,0,0,0.04)';

const mFont = "'Inter', system-ui, sans-serif";
const mFontMono = "'JetBrains Mono', 'Fira Mono', monospace";

// ── icon helper ───────────────────────────────────────────────
function MIco({ d, size = 18, stroke = mText, sw = 1.75, fill = 'none' }) {
  return (
...
const MSearch   = p => <MIco {...p} d={<><circle cx="11" cy="11" r="7" /><path d="M21 21l-4.3-4.3" /></>} />;
const MHeart    = p => <MIco {...p} d={<path d="M12 20s-7-4.5-9.2-9C1.3 8 3 4.5 6.3 4.5c2 0 3.2 1.3 3.7 2.2.5-.9 1.7-2.2 3.7-2.2C20 4.5 21.7 8 21.2 11 19 15.5 12 20 12 20z" />} />;
const MPin      = p => <MIco {...p} d={<><path d="M12 21s7-5.5 7-11a7 7 0 1 0-14 0c0 5.5 7 11 7 11z" /><circle cx="12" cy="10" r="2.5" /></>} />;
const MChevDown = p => <MIco {...p} d={<path d="M6 9l6 6 6-6" />} />;
const MChevRight= p => <MIco {...p} d={<path d="M9 6l6 6-6 6" />} />;
const MChevLeft = p => <MIco {...p} d={<path d="M15 18l-6-6 6-6" />} />;
const MMenu     = p => <MIco {...p} d={<><path d="M3 6h18" /><path d="M3 12h18" /><path d="M3 18h18" /></>} />;
const MClose    = p => <MIco {...p} d={<><path d="M18 6L6 18" /><path d="M6 6l12 12" /></>} />;
const MGrid2    = p => <MIco {...p} d={<><rect x="3" y="3" width="7" height="7" rx="1" /><rect x="14" y="3" width="7" height="7" rx="1" /><rect x="3" y="14" width="7" height="7" rx="1" /><rect x="14" y="14" width="7" height="7" rx="1" /></>} />;
const MGrid1    = p => <MIco {...p} d={<><rect x="3" y="4" width="18" height="6" rx="1" /><rect x="3" y="14" width="18" height="6" rx="1" /></>} />;
const MCamera   = p => <MIco {...p} d={<><path d="M23 19a2 2 0 0 1-2 2H3a2 2 0 0 1-2-2V7a2 2 0 0 1 2-2h4l2-3h6l2 3h4a2 2 0 0 1 2 2z" /><circle cx="12" cy="13" r="4" /></>} />;
const MPlus     = p => <MIco {...p} d={<><path d="M12 5v14" /><path d="M5 12h14" /></>} />;
const MCheck    = p => <MIco {...p} d={<path d="M20 6L9 17l-5-5" />} />;
const MHome     = p => <MIco {...p} d={<><path d="M3 11l9-8 9 8" /><path d="M5 10v10h14V10" /></>} />;
const MTag      = p => <MIco {...p} d={<><path d="M20.5 13.5L13 21l-9-9V4h8z" /><circle cx="8" cy="8" r="1.5" fill={mText} stroke="none" /></>} />;
const MInfo     = p => <MIco {...p} d={<><circle cx="12" cy="12" r="9" /><path d="M12 11v5" /><path d="M12 8h.01" /></>} />;
const MUser     = p => <MIco {...p} d={<><circle cx="12" cy="8" r="4" /><path d="M4 21c0-4 4-6 8-6s8 2 8 6" /></>} />;
const MUpload   = p => <MIco {...p} d={<><path d="M12 16V4" /><path d="M7 9l5-5 5 5" /><path d="M4 16v3a1 1 0 0 0 1 1h14a1 1 0 0 0 1-1v-3" /></>} />;
const MStar     = p => <MIco fill={mAccent} stroke="none" {...p} d={<path d="M12 2l3.09 6.26L22 9.27l-5 4.87 1.18 6.88L12 17.77l-6.18 3.25L7 14.14 2 9.27l6.91-1.01L12 2z" />} />;
const MGoogle   = () => (
...
function MButton({ children, variant = 'default', size = 'default', disabled, icon, iconRight, full, onClick, style }) {
  const pads    = { default: '0 16px', sm: '0 12px', lg: '0 24px', icon: '0' };
  const heights = { default: 40, sm: 36, lg: 44, icon: 40 };
  const widths  = { default: 'auto', sm: 'auto', lg: 'auto', icon: 40 };
  ...
  return (
    <button
      ...
      style={{
        ...
        fontWeight: 500,
        height: heights[size],
        width: size === 'icon' ? widths[size] : full ? '100%' : 'auto',
        minWidth: size === 'icon' ? widths[size] : undefined,
        padding: pads[size],
        borderRadius: mRadiusSm,
...
// ── Input ─────────────────────────────────────────────────────
function MInput({ label, placeholder, value, type = 'text', state = 'default', hint, prefix, suffix, style }) {
  const borderColor = state === 'error' ? mAccent : state === 'focus' ? mText : mBorder;
  const shadow      = state === 'focus' ? `0 0 0 3px ${mRing}` : 'none';
  return (
    <div style={{ ...style }}>
      {label && (
        <label style={{ display: 'block', fontFamily: mFont, fontSize: 13, fontWeight: 500, color: mText, marginBottom: 6 }}>
          {label}
        </label>
      )}
...
// ── Textarea ──────────────────────────────────────────────────
function MTextarea({ label, placeholder, value, rows = 3, count, max, style }) {
  return (
    <div style={{ ...style }}>
...
function MBadge({ children, variant = 'secondary', icon, style }) {
  const skins = {
    default:     { background: mText, color: mWhite, border: 'none' },
...
// ── SegmentedControl (toggle bar) ─────────────────────────────
function MSegmented({ options, active = 0, style }) {
  return (
    <div style={{ display: 'inline-flex', padding: 3, borderRadius: mRadius, border: `1px solid ${mBorder}`, background: mMutedBg, gap: 2, ...style }}>
      {options.map((o, i) => (
        <span key={o} style={{
          ...
          fontFamily: mFont, fontSize: 13, fontWeight: i === active ? 500 : 400,
          padding: '5px 14px', borderRadius: `calc(${mRadius} - 2px)`,
          background: i === active ? mWhite : 'transparent',
          color: i === active ? mText : mMuted,
          boxShadow: i === active ? mShadow : 'none',
...
// ── SearchBar (items-page header) ─────────────────────────────
function MSearchBar({ value, grid = 2, style }) {
  return (
...
// ── Upload ────────────────────────────────────────────────────
function MUploadImage({ state = 'empty', style }) {
  if (state === 'filled') {
    return (
...
// ── Card wrapper ──────────────────────────────────────────────
function MCard({ children, style, pad = '20px' }) {
  return (
...
// ── Separator ─────────────────────────────────────────────────
function MSeparator({ style }) {
  return <div style={{ height: 1, background: mBorder, ...style }} />;
}

Object.assign(window, {
  // tokens
  mBg, mWhite, mBorder, mBorder2, mText, mSubtext, mMuted, mMutedBg, mMutedBg2,
  mAccent, mAccentHov, mAccentBg, mRing, mRadius, mRadiusSm, mRadiusLg,
  mShadow, mShadowMd, mFont, mFontMono,
  // icons
  MIco, MSearch, MHeart, MPin, MChevDown, MChevRight, MChevLeft, MMenu, MClose,
  MGrid2, MGrid1, MCamera, MPlus, MCheck, MHome, MTag, MInfo, MUser, MUpload,
  MStar, MGoogle,
  // atoms
  MButton, MInput, MTextarea, MBadge, MSegmented, MSearchBar, MUploadImage, MCard, MSeparator,
});
