...
import { ArrowLeft, Camera, Plus, X, MapPin, Check } from 'lucide-react'
...
const CONDITIONS = ['Brand new', 'Like new', 'Good', 'Used', 'For parts']
...
export default function Upload({ onBack, onSuccess, initialItem }) {
  ...
  const handleFileChange = e => {
    ...
    const toAdd = chosen.slice(0, remaining).map(f => ({ url: URL.createObjectURL(f), name: f.name }))
        ...
        <div className="flex flex-col gap-3 w-full max-w-xs">
          {!isEdit && (
            <Button className="w-full h-12" onClick={() => {
              ...
              setDesc(''); setPrice(''); setPostcode(''); resetFiles()
            }}>
              List another item
            </Button>
          )}
      ...
      <div className="max-w-screen-lg mx-auto px-5 py-6 pb-24 space-y-7">
        ...
        <section>
          ...
          {files.length === 0 ? (
            ...
            <div className="flex gap-2 flex-wrap">
              {files.map((f, i) => (
                <div key={f.url} className="relative w-24 h-24 rounded-xl overflow-hidden shrink-0 bg-zinc-100">
        ...
        <section className="space-y-5">
          ...
          <div>
            ...
            <div className="flex gap-2 flex-wrap">
              {CONDITIONS.map(c => (
                <Pill key={c} active={condition === c} onClick={() => setCondition(c)}>{c}</Pill>
              ))}
            </div>
          </div>
        ...
        <div>
          <Button
            className={cn('w-full h-13 text-base transition-colors', !canSubmit && 'opacity-40')}
            style={{ height: 52 }}
            disabled={!canSubmit}
